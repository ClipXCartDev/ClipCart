...
| 11.16 | **Editor scope UPGRADED (2026-08-14, client):** from layer-only overlay to a **full-fledged editor** (trim/multi-track/transitions/filters via `pro_video_editor` BSD-3, the best free Flutter-native OSS editor). Must-have client features: **timed subtitles — multiple lines at different timestamps, each with own font/color/size** + **custom font upload** (`.ttf/.otf`, FontLoader preview → FFmpeg `drawtext fontfile=`). Export stays **on-device** via `ffmpeg_kit_flutter_new` (`drawtext enable='between(t,start,end)'` per line + `overlay`). Relaxes 11.13's "not a full editor" guard. Web/desktop OSS editors (OpenCut/OpenReel/Diffusion Studio) rejected — not embeddable in a Flutter mobile app. | supersedes 11.13 |
